...
def match(socket, plug):
    ...
    return socketColor == plugColor or socketShape == plugShape
...
class Node:
    ...
    def insert(self, node):
        if self.leftNode is None and match(self.leftType, node.plugType):
            self.leftNode = node
            return True
        ...
        if self.rightNode is None and match(self.rightType, node.plugType):
            self.rightNode = node
            return True
